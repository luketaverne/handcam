...
import os
import datetime
from handcam.ltt.network.model.Wide_ResNet import wide_resnet_tf_depth as resnet_model

# from handcam.ltt.network.model.Wide_ResNet import wide_resnet_tf_new as resnet_model_official
...
flags.DEFINE_string(
    "train_dir",
    "/tmp/luke/WRN_split/" + today + "/train",
    "Directory to keep training outputs.",
)
flags.DEFINE_string(
    "eval_dir",
    "/tmp/luke/WRN_split/" + today + "/eval",
...
def _parse_function(example_proto):
    ...
    rgb_img = tf.image.per_image_standardization(img[..., 0:3])
    # depth_img = tf.image.per_image_standardization(img[...,3:])

    std_img = tf.concat([rgb_img, img[..., 3:]], axis=2)

    one_hot = tf.one_hot(
        parsed_features["image/class/label"], len(class_names), dtype=tf.int32
    )

    return std_img, one_hot
...
for i in range(2, len(test_splits)):
    print("K-fold " + str(i))
    ...
    # Restore variables.
    # if restore_sess:
    #     checkpoint_path = tf.train.latest_checkpoint(config['model_dir'] + "split_" + str(i))
