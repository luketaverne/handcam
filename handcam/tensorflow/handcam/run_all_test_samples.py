...
from handcam.ltt.util.Utils import handcam_gesture_spotting_acc, AttrDict
...
# resnet50_split0_results = {
#     "oldloc_newobj_single": {
#         'rgb': {'accuracy': 0.8322,
#                 'gesture_spotting': 0.9845},
#         'depth': {'accuracy': 0.9195,
#                   'gesture_spotting': 0.9868},
#         'rgbd': {'accuracy': 0.8392,
#                  'gesture_spotting': 0.9827}
#     },
#     "newloc_oldobj_single": {
#         'rgb': {'accuracy': 0.7397,
#                 'gesture_spotting': 0.9692},
#         'depth': {'accuracy': 0.6689,
#                   'gesture_spotting': 0.9368},
#         'rgbd': {'accuracy': 0.7334,
#                  'gesture_spotting': 0.9676}
#     },
#     "newloc_newobj_single": {
#         'rgb': {'accuracy': 0.6597,
#                  'gesture_spotting': 0.9421},
#         'depth': {'accuracy': 0.6287,
#                  'gesture_spotting': 0.9269},
#         'rgbd': {'accuracy': 0.6379,
#                  'gesture_spotting': 0.9392}
#     }
#
# }
...
for resnet_type in compiled_results_dict.keys():
    for model_type in compiled_results_dict[resnet_type].keys():
        for test_type in test_types.keys():
            compiled_results_dict[resnet_type][model_type][test_type] = {}
            for split_id in range(0, 10):
                compiled_results_dict[resnet_type][model_type][test_type][
                    "split%d" % split_id
                ] = {}
                if split_id == 0 and resnet_type == "resnet-50":
                    # compiled_results_dict[resnet_type][model_type][test_type]['split%d' % split_id] = resnet50_split0_results[test_type]
...
for test_type in test_types.keys():
    test_types[test_type] = glob.glob(
        os.path.join(
    ...
    print("Found %d of %s" % (len(test_types[test_type]), test_type))
...
for model_path in models_to_eval.keys():
    ...
    # need to modify flags_dict a bit
    FLAGS.batch_size = 1
    ...
    print("Begin testing on: %s" % model_path)

    # init
    for test_type in test_types.keys():
        # print("Testing %s on %s" % (model_path, test_type))
        ...
        test_feed_dict = {filenames_placeholder: test_types[test_type]}
        coord = tf.train.Coordinator()
        threads = tf.train.start_queue_runners(sess=sess, coord=coord)
        ...
        with open(os.path.join(model_path, test_type + "_results.pckl"), "wb") as f:
            pickle.dump(out_dict, f)

        print(
            "per frame: %.2f\tgesture spotting: %.2f"
            % (100 * out_dict["val_accuracy"], 100 * spotting_accuracy)
        )
        ...
        if FLAGS.model_type != "single_frames":
            dict_model_type = (
                "sequence_frozen"
                if FLAGS.mode == "frozen_train"
                else "sequence_end2end"
            )

        dict_split_name = "split%d" % FLAGS.validation_split_num
        dict_modality = FLAGS.input_modality

        compiled_results_dict[dict_resnet_type][dict_model_type][test_type][
            dict_split_name
        ][dict_modality] = {
    ...
    sess.close()
    print("End testing on: %s\n\n\n" % model_path)

with open("/local/home/luke/all_test_cases_ICRA.pckl", "wb") as f:
