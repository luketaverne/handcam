...
desired_model_dir = "2018-04-09/12:42/"
checkpoint_id = None  # None or a number
...
config["resnet_dir"] = "/tmp/luke/handcam/WRN/2018-04-07/15:51"
config["seq_len"] = 60
...
config["cnn"][
    "num_hidden_units"
] = 1024  # Number of output units, i.e. representation size.
config["cnn"]["dropout_rate"] = 0.0
config["cnn"]["initializer"] = tf.contrib.layers.xavier_initializer()
config["rnn"] = {}
config["rnn"]["num_hidden_units"] = 2048  # Number of units in an LSTM cell.
...
with open(os.path.join(FLAGS.dataset_dir, "validation_split0.pckl"), "rb") as f:
...
for filename in all_filenames:
    for validation_sample in validation_samples:
        if validation_sample in filename:
            ...
            test_tfrecord_filenames.append(filename)
            break
...
config["num_steps_per_epoch"] = len(test_tfrecord_filenames) / FLAGS.batch_size
...
def _parse_function(example_proto):
    ...
    img = tf.image.resize_image_with_crop_or_pad(img[:, 40:280], 112, 112)
...
def trim_sequence(seq_img, seq_labels, seq_len):
    ...
    return seq_img, seq_labels, seq_len
...
def read_and_decode_sequence(filename_queue):
    ...
    with tf.name_scope("TFRecordDecoding"):
        (
            ...
            last_grasp_frame,
            sample_name,
        ) = _parse_function(filename_queue)
        # print(seq_labels.shape)
        # seq_img = seq_img[first_grasp_frame:last_grasp_frame]
        # seq_labels = seq_labels[first_grasp_frame:last_grasp_frame]
        # seq_len = tf.shape(seq_img)[0]

        # seq_img, seq_labels, seq_len = tf.cond(seq_len - config['seq_len'] > 0, lambda: trim_sequence(seq_img, seq_labels, seq_len), lambda: (seq_img, seq_labels, seq_len))
        ...
        seq_img = per_sequence_standardization(seq_img)
        # seq_img = per_sequence_standardization_rgbd(seq_img)
...
if checkpoint_id is None:
    checkpoint_id = tf.train.latest_checkpoint(config["model_dir"])

checkpoint_path = os.path.join(config["model_dir"], checkpoint_id)
