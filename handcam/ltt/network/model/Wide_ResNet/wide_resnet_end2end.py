...
def conv2d_fixed_padding(
    inputs, filters, training, kernel_size, strides, data_format, name_func, reuse
):
...
class Model(object):
    ...
    def __init__(
        self, seq_input_op, seq_labels_op, seq_len_op, training, config, num_classes=7
    ):
        self.is_training = training
        self.reuse = not self.is_training
        self.seq_input = seq_input_op
        self.seq_labels = seq_labels_op
        self.seq_len = seq_len_op
        ...
        # setup (need to use 1 image to get the network shape correct)
        self.model = self.__call__(self.seq_input[0], training=self.is_training)
        ...
        self.cnn_representations = tf.map_fn(
            lambda x: self.__call__(x, training=self.is_training),
            elems=self.seq_input,
            dtype=tf.float32,
            back_prop=self.is_training,
            swap_memory=True,
        ...
        self.model = lstm_model(
            ...
            target_op=self.seq_labels,
            seq_len_op=self.seq_len,
    ...
    def __call__(self, inputs, training):
        ...
        with tf.variable_scope("WRN"):
            with tf.variable_scope("rgb"):
                ...
                for i, num_blocks in enumerate(self.block_sizes):
                    ...
                    rgb = block_layer(
                        ...
                        training=training,
                        name="block_layer_rgb{}".format(i + 1),
                        data_format=self.data_format,
                        reuse=self.reuse,
                        name_func=self._get_batch_norm_name,
                        name_func_conv=self._get_conv_name,
                    )
            ...
            with tf.variable_scope("depth"):
                ...
                for i, num_blocks in enumerate(self.block_sizes):
                    ...
                    depth = block_layer(
                        ...
                        strides=self.block_strides[i],
                        training=training,
                        name="block_layer_depth{}".format(i + 1),
                        data_format=self.data_format,
                        reuse=self.reuse,
                        name_func=self._get_batch_norm_name,
                        name_func_conv=self._get_conv_name,
            ...
            out = tf.layers.dense(
                inputs=out, units=1024, reuse=self.reuse, name="final_dense"
            )
